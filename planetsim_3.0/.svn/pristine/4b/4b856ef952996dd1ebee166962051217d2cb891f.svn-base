...
public class ScribeTest extends TestCase {
	...
	protected void setUp() {
		...
		try {
			// arguments: properties file, master property, application level,
			// events, results, serialization
			GenericApp.start("../conf/master.properties",
					TestNames.SCRIBE_SCRIBETEST, true, false, false, false);
			/*
			 * if (!(Properties.overlayPropertiesInstance instanceof
			 * ChordProperties)) { System.out.println(
			 * "Only the Chord overlay can be configured for this test.");
			 * System.exit(-1); }
			 * 
			 * Properties.simulatorQueueSize=250;
			 * ((ChordProperties)Properties.overlayPropertiesInstance
			 * ).stabilizeSteps=2;
			 * ((ChordProperties)Properties.overlayPropertiesInstance
			 * ).fixFingerSteps=2;
			 * 
	...
	public void testSend() throws Exception {
		...
		System.out.println("Application has been register in " + network.size()
				+ " nodes ");
